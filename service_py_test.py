...
def test_retrieveById():
    with db_session:
        # Insere uma pessoa para garantir que há dados
        response_insert = insert("Lucas Mendes", "lucas.mendes@example.com", "333333333")
        pessoa_id = response_insert["details"]
        pessoa = retrieveById(pessoa_id)
        rollback()
        assert pessoa["result"] == "ok"
        assert pessoa["details"]["id"] == pessoa_id
        assert pessoa["details"]["nome"] == "Lucas Mendes"
        assert pessoa["details"]["email"] == "lucas.mendes@example.com"
        assert pessoa["details"]["telefone"] == "333333333"
        # Tenta recuperar uma pessoa que não existe
        pessoa_nonexistent = retrieveById(99999)
        assert pessoa_nonexistent["result"] == "error"
